...
class SysdigProcessor:
    ...
    def split_scap_file(self) -> int:
        ...
        output_prefix = self.capture_dir / self.base_name
        command = [
            "sysdig",
        ...
        idx = 0
        while True:
            scap_file = self.capture_dir / f"{self.base_name}.scap{idx}"
            if scap_file.exists():
    ...
    def process_single_scap(args):
        ...
        idx, capture_dir, base_name, script_dir = args
        
        scap_path = capture_dir / f"{base_name}.scap{idx}"
        gz_path = capture_dir / f"{base_name}.{idx}.gz"
    ...
    def process_scap_parallel(self, num_files: int):
        ...
        for idx in range(num_files):
            gz_file = self.capture_dir / f"{self.base_name}.{idx}.gz"
            self.temp_files.append(gz_file)
        
        # Prepare arguments for all files
        args_list = [
            (idx, self.capture_dir, self.base_name, script_dir) 
            for idx in range(num_files)
        ]
    ...
    def load_gz_file(self, idx: int) -> Generator[Dict, None, None]:
        ...
        gz_path = self.capture_dir / f"{self.base_name}.{idx}.gz"
    ...
    def load_all_events(self) -> Generator[Dict, None, None]:
        ...
        idx = 0
        while (self.capture_dir / f"{self.base_name}.{idx}.gz").exists():
            idx += 1
