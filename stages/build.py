...
def build_plugins(cve):
    ...
    if cve.language == "php":
        ...
        subprocess.run(
            ["sudo", "bash", f"stack_tracer/php{cve.version}/source/build.sh",
             str(env_dir), cve.container],
            cwd=project_root, check=True,
        )

        logger.info("Building syscall_filter...")
        subprocess.run(
            ["sudo", "bash", f"syscall_filter/php{cve.version}/source/build.sh",
             str(env_dir), cve.container],
            cwd=project_root, check=True,
        )
        ...
        subprocess.run(
            ["sudo", "bash", "net_filter/setup.sh", cve.container],
            cwd=project_root, check=True,
        )
        subprocess.run(
